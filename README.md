...
| CustomerName | | 客戶名稱 | String (60)  | 當列印註記[Print]=1(列印)時，此參數為必填 |
...
| InvoiceDate | ✔ | 發票號碼 | String (20) | 格式為「yyyy-MM-dd」或 「yyyy/MM/dd」                                                    |
| PrintStyle | | 發票列印格式 | int         | 1：一般列印(單面)，預設2：一般列印(雙面)3：熱感應紙列印 4：B2B A4(僅限帶有統編的發票才能使用) 5: B2B A5(僅限帶有統編的發票才能使用) |
| IsReprintInvoice | | 是否顯示補印發票 | String (1)  | Y：顯示為”電子發票證明聯補印”                                                                 |
| IsShowingDetail | | 是否顯示明細 | int | 1：顯示明細 2：隱藏明細                                                                    |